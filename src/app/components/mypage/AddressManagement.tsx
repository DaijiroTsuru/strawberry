...
const emptyAddress: AddressInput = {
  lastName: '', firstName: '', company: '', zip: '', province: '', city: '', address1: '', address2: '', phone: '', country: 'JP',
};
...
export function AddressManagement() {
  ...
  const openNewForm = () => { clearError(); setEditingAddress({ ...emptyAddress }); setIsNew(true); setFormErrors({}); };

  const openEditForm = (addr: ShopifyAddress) => {
    clearError();
    setEditingAddress({
      id: addr.id, lastName: addr.lastName || '', firstName: addr.firstName || '', company: addr.company || '',
      zip: addr.zip || '', province: addr.province || '', city: addr.city || '', address1: addr.address1 || '',
      address2: addr.address2 || '', phone: addr.phone || '', country: 'JP',
    });
    setIsNew(false); setFormErrors({});
  };

  const closeForm = () => { setEditingAddress(null); setFormErrors({}); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) { await createAddress(input); setSuccess('住所を追加しました'); }
      else if (id) { await updateAddress(id, input); setSuccess('住所を更新しました'); }
      closeForm(); setTimeout(() => setSuccess(null), 3000);
    } catch {} finally { setSubmitting(false); }
  };

  const handleDelete = async (id: string) => {
    if (!window.confirm('この住所を削除しますか？')) return;
    setDeletingId(id); clearError();
    try { await deleteAddress(id); setSuccess('住所を削除しました'); setTimeout(() => setSuccess(null), 3000); }
    catch {} finally { setDeletingId(null); }
  };

  const handleSetDefault = async (id: string) => {
    setSettingDefaultId(id); clearError();
    try { await setDefaultAddress(id); setSuccess('デフォルト住所を設定しました'); setTimeout(() => setSuccess(null), 3000); }
  ...
  const inputStyle = (field: string) => ({
    ...
    fontFamily: 'var(--font-sans)', backgroundColor: 'white',
  });
  ...
  return (
    <div className="space-y-6">
      {error && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-2xl flex items-center gap-3" style={{ background: 'linear-gradient(135deg, #fee2e2 0%, #fecaca 100%)', border: '1px solid #fca5a5' }}>
      ...
      {success && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-2xl flex items-center gap-3" style={{ background: 'linear-gradient(135deg, #d1fae5 0%, #a7f3d0 100%)', border: '1px solid #6ee7b7' }}>
      ...
      <button onClick={openNewForm} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 hover:opacity-80" style={{ background: 'linear-gradient(135deg, var(--color-strawberry-600) 0%, var(--color-strawberry-700) 100%)', color: 'white', fontFamily: 'var(--font-sans)', boxShadow: '0 2px 8px rgba(220, 38, 38, 0.2)' }}>
        <Plus className="w-4 h-4" />新しい住所を追加
      </button>

      {addresses.length === 0 && !editingAddress && (
        <div className="text-center py-16">
          <MapPin className="w-16 h-16 mx-auto mb-4" style={{ color: 'var(--color-neutral-300)' }} />
          <p className="text-lg font-medium mb-2" style={{ fontFamily: 'var(--font-sans)', color: 'var(--color-neutral-600)' }}>登録済みの住所がありません</p>
        </div>
      )}

      <div className="grid gap-4">
        {addresses.map((addr) => (
          <div key={addr.id} className="rounded-2xl p-6" style={{ background: 'white', border: '1px solid var(--color-neutral-200)', boxShadow: '0 1px 3px rgba(0, 0, 0, 0.05)' }}>
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                {addr.id === defaultAddressId && (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium mb-2" style={{ background: 'var(--color-strawberry-50)', color: 'var(--color-strawberry-700)', border: '1px solid var(--color-strawberry-200)' }}>
                ...
                <p className="text-sm mt-1" style={{ fontFamily: 'var(--font-sans)', color: 'var(--color-neutral-600)' }}>
                  〒{addr.zip} {addr.province}{addr.city}{addr.address1}{addr.address2 && ` ${addr.address2}`}
                </p>
                {addr.phone && <p className="text-sm mt-1" style={{ fontFamily: 'var(--font-sans)', color: 'var(--color-neutral-500)' }}>TEL: {addr.phone}</p>}
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                {addr.id !== defaultAddressId && (
                  <button onClick={() => handleSetDefault(addr.id)} disabled={settingDefaultId === addr.id} className="p-2 rounded-lg transition-colors duration-300 hover:bg-[color:var(--color-neutral-100)]" style={{ color: 'var(--color-neutral-500)' }} title="デフォルトに設定">
                    {settingDefaultId === addr.id ? <div className="w-4 h-4 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: 'var(--color-neutral-400)', borderTopColor: 'transparent' }} /> : <Star className="w-4 h-4" />}
                  </button>
                )}
                <button onClick={() => openEditForm(addr)} className="p-2 rounded-lg transition-colors duration-300 hover:bg-[color:var(--color-neutral-100)]" style={{ color: 'var(--color-neutral-500)' }} title="編集"><Pencil className="w-4 h-4" /></button>
                <button onClick={() => handleDelete(addr.id)} disabled={deletingId === addr.id} className="p-2 rounded-lg transition-colors duration-300 hover:bg-[color:var(--color-strawberry-50)]" style={{ color: 'var(--color-strawberry-500)' }} title="削除">
                  {deletingId === addr.id ? <div className="w-4 h-4 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: 'var(--color-strawberry-400)', borderTopColor: 'transparent' }} /> : <Trash2 className="w-4 h-4" />}
                </button>
              </div>
            </div>
          </div>
      ...
      {editingAddress && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0, 0, 0, 0.5)' }}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-3xl p-8" style={{ background: 'white', boxShadow: '0 25px 50px rgba(0, 0, 0, 0.15)' }}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold" style={{ fontFamily: 'var(--font-serif)', color: 'var(--color-neutral-900)' }}>{isNew ? '新しい住所を追加' : '住所を編集'}</h3>
              <button onClick={closeForm} className="p-2 rounded-lg transition-colors duration-300 hover:bg-[color:var(--color-neutral-100)]" style={{ color: 'var(--color-neutral-500)' }}><X className="w-5 h-5" /></button>
            </div>
            <div className="space-y-4">
              ...
              <div>
                <label className="block mb-1 text-sm font-medium" style={{ fontFamily: 'var(--font-sans)', color: 'var(--color-neutral-700)' }}>郵便番号 <span style={{ color: 'var(--color-strawberry-600)' }}>*</span></label>
              ...
              <div>
                <label className="block mb-1 text-sm font-medium" style={{ fontFamily: 'var(--font-sans)', color: 'var(--color-neutral-700)' }}>都道府県 <span style={{ color: 'var(--color-strawberry-600)' }}>*</span></label>
              ...
              <div>
                <label className="block mb-1 text-sm font-medium" style={{ fontFamily: 'var(--font-sans)', color: 'var(--color-neutral-700)' }}>市区町村 <span style={{ color: 'var(--color-strawberry-600)' }}>*</span></label>
              ...
              <div>
                <label className="block mb-1 text-sm font-medium" style={{ fontFamily: 'var(--font-sans)', color: 'var(--color-neutral-700)' }}>住所1 <span style={{ color: 'var(--color-strawberry-600)' }}>*</span></label>
            ...
            <div className="flex items-center gap-3 mt-8">
              <motion.button onClick={handleSave} disabled={submitting} whileHover={{ scale: submitting ? 1 : 1.02 }} whileTap={{ scale: submitting ? 1 : 0.98 }} className="px-6 py-2.5 rounded-xl font-semibold transition-all duration-300 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed text-sm" style={{ background: submitting ? 'var(--color-neutral-400)' : 'linear-gradient(135deg, var(--color-strawberry-600) 0%, var(--color-strawberry-700) 100%)', color: 'white', fontFamily: 'var(--font-sans)', boxShadow: submitting ? 'none' : '0 4px 12px rgba(220, 38, 38, 0.3)' }}>
                {submitting ? (<><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />保存中...</>) : (<><Save className="w-4 h-4" />保存する</>)}
              </motion.button>
              <button onClick={closeForm} className="px-6 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 hover:bg-[color:var(--color-neutral-100)]" style={{ color: 'var(--color-neutral-600)', fontFamily: 'var(--font-sans)', border: '1px solid var(--color-neutral-300)' }}>キャンセル</button>
            </div>
          </motion.div>
        </div>
      )}
